...
class _EditCoursePageState extends State<EditCurso> {
  ...
  void didChangeDependencies() {
    ...
    _descriptionController = TextEditingController(text: arguments['description'] ?? '');
    _instructorIdController = TextEditingController(text: arguments['instructor_id'] ?? '');
    _priceController = TextEditingController(text: arguments['price']?.toString() ?? '0.0');
    _categoryController = TextEditingController(text: arguments['category'] ?? '');
    _uid = arguments['uid'] ?? '';

    print("Valores recibidos:");
print("Título: ${_titleController.text}");
print("Descripción: ${_descriptionController.text}");
print("Instructor ID: ${_instructorIdController.text}");
print("Precio: ${_priceController.text}");
print("Categoría: ${_categoryController.text}");
print("UID: $_uid");
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Editar Curso", style: TextStyle(color: Colors.white)),
            ...
            child: Column(
              children: [
                ...
                ElevatedButton(
                  ...
                  child: const Text('Actualizar Curso', style: TextStyle(color: Colors.white), ),
